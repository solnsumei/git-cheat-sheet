...
class CategoryPage extends React.Component {
  ...
  saveCommand = (event) => {
    ...
    if (command.snippet.trim().length < 3) {
      error.snippet = 'Field cannot be less that 3 chars';
    }
    if (command.snippet.trim().length > 60) {
      error.snippet = 'Field cannot be more than 60 chars';
    }

    if (command.action.trim().length < 3) {
      error.action = 'Field cannot be less that 3 chars';
    }

    if (command.action.trim().length > 100) {
      error.action = 'Field cannot be more than 100 chars';
    }
